...
public class TakeSubject { // find name, time, so tin with regex
	...
	public static boolean takeTime(String line) { // A problem
		...
		//Pattern regexFindTime = Pattern.compile("\\w{1}\\d{1}[-][(]\\d{1}[-]\\d{1,2}[)][-](\\w{2,3}(\\s)?)+"); //T3-(6-8)-KTX M? Trì 
		//Pattern regexFindTimeOth = Pattern.compile("(\\w{1}\\d{1}[-][(]\\d{1}[-]\\d{1,2}[)][-]\\d{3}\\w{1}\\d{1}[,]?)+\\w{1}\\d{1}[-][(]\\d{1}[-]\\d{1,2}[)][-](\\w{3,5}(\\s)?)+(\\d{1})?"); // T4-(1-5)-Phòng máy 1
